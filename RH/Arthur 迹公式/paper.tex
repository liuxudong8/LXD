...
\usepackage{hyperref}   % 处理邮箱 \href
\usepackage{enumitem}

\makeatletter
\renewenvironment{thebibliography}[1]
     {\section*{参考文献}
      % 只设置本部分页眉，参考文献结束立刻恢复，不要全局污染后续页面
      \@mkboth{}{}
...
\textbf{《基于 Arthur 稳定迹、Jacquet--Langlands
对应与测地流指数混合的谱对偶论证9》}

\textbf{修改依据：结合刚刚定稿的BSD 定性论文的审稿过程：}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  自伴算子离散本征值本身已是实数，不能把 Dolgopyat 混合当作
  ``证明谱参数为实数'' 的主要论据；Dolgopyat
  仅作为几何层面的补充兜底，主要论证依靠：自伴算子谱定理 +
  Jacquet‑Langlands 酉等价传递；
\item
  区分''离散谱''与''连续谱''，Anosov 流 / Ruelle‑Zeta 零点 ≠ Laplacian
  离散本征值，避免跨域概念混淆；
\item
  澄清：保序双射只是集合层面一一对应，不自动等价 Selberg zeta 与
  Dedekind‑ζ 的全域解析恒等；
\item
  修正局部域 JL 提升的措辞：\(w_{p} = 1/2\) 是局部酉投影归一因子，不是
  ``人为抵消权重''；
\item
  符号、笔误、等式排版清理；
\item
  明确论证边界：本文得到的是对于紧支磨光测试函数的加权求和等价，不是得到
...
Correspondence and Exponential Mixing of Geodesic Flows: 
Remarks on the Critical Line of Non‑Trivial Zeros of $\zeta(s)$
...
早期研究尝试建立 \({PSL}_{2}\left( \mathcal{O}_{K} \right)\)
算术三维双曲 orbifold 的 Selberg zeta
函数与黎曼\(\zeta\)函数的\textbf{全域解析恒等}，但对实二次域
...
依托系列第二篇《$\mathrm{PSL}_{2}\left(\mathcal{O}_{K}\right)$ 本原闭测地线与$\mathbb{Q}\left(\sqrt{5}\right)$素理想保序双射定理》给出集合层面几何‑数论一一映射
\[
\ell\left(\gamma\right)=\log\mathrm{Nm}\left(\mathfrak{p}\right),
\]
论证链：
...
\def\labelenumi{\arabic{enumi}.}
\item
  给出算术三维 orbifold 上 Arthur
  稳定迹完整分解：分离双曲本原轨道主项、有限椭圆共轭修正项、尖点连续谱散射贡献；
\item
  利用保序双射完成迹公式几何侧变量替换，严格化简三维双曲轨道行列式权重；对分裂素，局部域直积分解造成几何侧二重轨道，由
  JL 局部归一权重逐项补偿；
\item
  通过 Jacquet‑Langlands 酉提升，建立三维流形尖点离散谱与
  \({PSL}_{2}\left( \mathbb{Z} \right)\) 模曲面 Maass
  尖点谱的\textbf{酉等价关系}；
\item
  核心：Laplacian 是 \(L^{2}\) 上无界自伴算子，离散本征值自动为实数；JL
  酉等价将实谱性质传递到三维 orbifold；Dolgopyat
  指数混合仅作为几何兜底约束，排除复周期轨道这种经典几何反例，不作为谱实值的主证明；
\item
  选取支集分离型紧支磨光测试函数，屏蔽短轨道椭圆扰动，把加权迹等式约化为黎曼‑von‑Mangoldt
...
\textbf{1.2 前置核心结论（系列第二篇主定理，措辞边界）}
...
记 \(\mathcal{G}\)为 M 全体本原双曲闭测地线集合；\(\mathfrak{I}_{prim}\)
为 \(\mathcal{O}_{K}\) 中非零本原素理想集合。 存在保序双射
...
\def\labelenumi{\arabic{enumi}.}
\item
  写出三维算术 orbifold 完整 Arthur 稳定迹分解，包含标准双曲轨道权重；
\item
  代入\(\Phi\)做几何侧变量替换，化简\(\left| \det\left( I - P_{\gamma} \right) \right|\)，分析分裂素局部多重轨道与
  JL 局部归一权重的逐项补偿；
\item
  借助 Jacquet‑Langlands\textbf{酉等价}，建立三维流尖点谱与模曲面 Maass
  尖点谱的对应；
\item
  主论证依靠：自伴算子谱定理 + JL
  ...
  指数混合作为几何兜底（不是主要证明手段）；
\item
  构造\textbf{支集分离型紧支磨光测试函数}，屏蔽椭圆短轨道扰动；将迹等式约化为黎曼‑von‑Mangoldt
  有限截断显式公式，导出黎曼猜想；
\item
  给出向全体实二次域推广的标准化流程。
\end{enumerate}
...
\def\labelenumi{\arabic{enumi}.}
\item
  \(K = \mathbb{Q}\left( \sqrt{5} \right)\),\(\ \mathcal{O}_{K} = \mathbb{Z}\left\lbrack \frac{1 + \sqrt{5}}{2} \right\rbrack\),\(\ \Gamma = {PSL}_{2}\left( \mathcal{O}_{K} \right)\),\(\ M = \Gamma\backslash\mathbb{H}^{3}\)；
\item
  \(X = {PSL}_{2}\left( \mathbb{Z} \right)\backslash\mathbb{H}^{2}\)：标准模曲面；
\item
  \(\mathcal{G}\)：M全体本原双曲闭测地线；\(\ell\left(\gamma\right)\)轨道双曲长度；
\item
  \(\mathcal{E}\)：\(\Gamma\)椭圆共轭类集合（有限集合，对应有限阶元）；
\item
  \(\mathfrak{I}_{prim}\)：\(\mathcal{O}_{K}\)本原素理想，\(Nm\left( \mathfrak{p} \right)\)理想范数；
\item
  \(\Delta_{M}\)：\(L^{2}(M)\)上无界自伴 Laplace‑Beltrami
  算子；\(Spec_{disc}(M)\)离散谱；
\item
  \(f \in C_{c}^{\infty}\left( \mathbb{R}_{> 0} \right)\)：光滑紧支测试函数；
\item
  \(P_{\gamma}\)：双曲等距\(\gamma\)在轨道稳定子的垂直切空间上的线性
  Poincaré 映射。
\end{enumerate}

\textbf{2.2 三维 orbifold Arthur 稳定迹标准等式}
...
\def\labelenumi{\arabic{enumi}.}
\item
  左侧：Laplacian 离散本征值加权求和；
\item
  右侧第一项（主项）：全体本原双曲轨道加权求和，标准 Selberg‑Arthur
  轨道权重；
\item
  第二项
...
\textbf{3.2 分裂素局部多重轨道与 JL 局部归一补偿}
...
\textbf{4 Jacquet‑Langlands 谱加权等价与谱实值性}

\textbf{4.1 JL 酉等价与加权迹等式}
...
\textbf{4.2 离散谱实值的主证明（不再把 Dolgopyat 当作主论据）}
...
\def\labelenumi{\arabic{enumi}.}
\item
  \(\Delta_{X}\)（模曲面）自伴，Maass 离散本征值
  \(\lambda = \frac{1}{4} + t^{2}\)，自动
  \(\lambda \in \mathbb{R}\)，于是 \(t \in \mathbb{R}\)。
\item
  JL 提升U是酉映射：
\end{enumerate}
...
\textbf{4.3 Dolgopyat 指数混合：仅作为几何兜底补充（不做主证明）}
...
\textbf{5 测试函数选取与黎曼 ζ 零点临界推论}

\textbf{5.1 支集分离型磨光测试函数}
...
由本征值参数化
\(\lambda = \frac{1}{4} + t^{2},\ t \in \mathbb{R}\)，定义 ζ 非平凡零点
...
\item
  复刻系列第二篇，得到素理想‑本原轨道保序双射；类数\(h_{F} > 1\)时改为理想类群上射影双射；
\item
  写出 \(M_{F}\) 的 Arthur 稳定迹；利用对应二次特征的 JL
  局部归一权重补偿分裂素多重轨道；
\item
  依靠自伴算子谱定理 + JL 酉等价得到离散谱实值；Dolgopyat 仅做几何补充；
\item
  选取支集分离磨光测试函数，得到该数域 Dedekind‑ζ 的局部 GRH 结论。
\end{enumerate}
...
\def\labelenumi{\arabic{enumi}.}
\item
  摒弃 \(Z_{M}(s) = \zeta(s)\) 全域恒等的错误路径；改用 Arthur
  迹的紧支加权求和范式，分裂素二重轨道由 JL
  局部酉投影归一权重逐项补偿，避开 Zeta 函数全局匹配的代数障碍。
\item
  以保序双射作为集合论基础，完整化简三维双曲轨道权重；逐项完成分裂素局部补偿。
\item
  谱实值严格证明：自伴算子谱定理 + Jacquet‑Langlands 酉等价；Dolgopyat
  指数混合仅作为几何兜底，不再作为核心证明，规避跨域概念误用。
\item
  使用支集分离磨光测试函数，屏蔽椭圆短轨道扰动；加权迹等式约化为有限截断黎曼‑von‑Mangoldt
  显式公式，得到标准黎曼猜想。
\item
  整套范式可推广全体实二次域，分步推进局部广义黎曼假设。
\end{enumerate}


\begin{thebibliography}{8}
\bibitem{1} Arthur J. Stable Trace Formula for Reductive Groups, \emph{Ann. Math.}, 1988.

\bibitem{2} Jacquet H, Langlands R. Automorphic Forms on GL(2), LNM 114, Springer.

\bibitem{3} Maclachlan C, Reid. The Arithmetic of Hyperbolic $3$-Manifolds, Springer.

\bibitem{4} Dolgopyat D. Mixing properties of geodesic flows on compact hyperbolic manifolds, \emph{Ann. Math.}, 1998.

\bibitem{5} Selberg A. Harmonic analysis and discontinuous groups, \emph{J. Indian Math. Soc.}, 1956.

\bibitem{6} Hejhal D. The Selberg Trace Formula for $\mathrm{PSL}_2(\mathbb R)$, Springer.

\bibitem{7} Ratcliffe J. Foundations of Hyperbolic Manifolds, GTM.

\bibitem{8} Iwaniec H. Topics in Classical Automorphic Forms, GTM.
\end{thebibliography}


\textbf{附录 A 支集分离磨光测试函数构造（与 BSD 论文统一）}

构造光滑截断 $\psi\in C_c^\infty(\mathbb R_{>0})$:
\[
\begin{cases}
\psi(x)=1, & x\in [\Lambda_0,\Lambda_1],\\
\psi(x)=0, & x\le \Lambda_0/2,
\end{cases}
\]
令 $f(x)=\psi(\log x)$。实现：长轨道完整保留，短椭圆轨道被屏蔽。磨光序列满足归一条件：
\[
\int_{\mathbb R} g_\varepsilon(u)\,du =1,\quad \lim_{\varepsilon\to 0}g_\varepsilon(0)=1,\quad \mathrm{supp}(g_\varepsilon)\subset(-\varepsilon,\varepsilon).
\]


\textbf{附录 B 三维轨道权重完整代数演算}
...
\def\labelenumi{\arabic{enumi}.}
\item
  本原双曲闭测地线 \(\gamma\)，对应素理想
  \(\mathfrak{p} = \Phi(\gamma)\)，范数
  \(N = Nm\left( \mathfrak{p} \right)\)；
\item
  轨道长度--范数恒等式：\(\ell\left(\gamma\right) = \log N\)；
\item
  三维双曲 orbifold 垂直切空间等距映射行列式：
\end{enumerate}
...
\textbf{步骤 2：计算} \(4\sinh^{2}\left( \frac{\log N}{2} \right)\)
...
\textbf{步骤 5：衔接黎曼--冯.曼戈尔特显式公式的匹配推导}

\textbf{5.1 迹等式整体形式（含离散谱、几何主项、椭圆、连续谱）}
...
% \(Re(\rho) = \frac{1}{2}\)。
模曲面 Maass 本征值标准形式: \(\lambda_{X} = \frac{1}{4} + t^{2}\)，定义黎曼 \(\zeta\) 非平凡零点 \(\rho = \frac{1}{2} + it\)，则
\[
t = i\left(\rho - \frac{1}{2}\right),\quad t \in \mathbb{R} \iff \operatorname{Re}(\rho) = \frac12
\]


\textbf{5.2 测试函数取对数磨光截断、求和转换为素计数}

取附录 A 构造的支集分离磨光函数
\(f\left( \ell \right) = \psi\left( \log\ell \right)\)，仅保留长轨道，椭圆项
\(\sum_{}^{}C_{\epsilon}(f) \rightarrow 0\)可舍弃；连续谱 \(Cont(f)\)
仅对应 ζ 平凡零点 \(s = - 2, - 4,\ldots\)，与非平凡零点位置无关。
...
\def\labelenumi{\arabic{enumi}.}
\item
  为什么分裂有理素会产生两条等长本原闭测地线（几何侧二重计数来源）；
\item
  局部域 \(K_{p} \cong \mathbb{Q}_{p} \oplus \mathbb{Q}_{p}\)
  直积分解，\({GL}_{2}\left( K_{p} \right)\) 的表示结构；
\item
  Jacquet‑Langlands 局部提升为什么自然带出归一因子
  \(w_{p} = \frac{1}{2}\)，不是人为拿来抵消的魔术系数；
\item
  逐项验算：几何二重轨道贡献，经过投影归一之后，和非分裂素局部贡献完全对齐；
\item
  区分：惯性素、分歧素，没有直积分解，故 \(w_{p} = 1\)，不需要修正。
\end{enumerate}
...
\textbf{D.1 前置：实二次域} \(K = \mathbb{Q}\left( \sqrt{5} \right)\)
\textbf{的素三分}
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{分歧素}：\(p = 5\)，\((5) = \mathfrak{p}^{2}\)，一个素理想，指数
  2；
\item
  \textbf{惯性（惰性）素}：\(p \equiv \pm 2\quad(mod\ 5)\)，\(p\mathcal{O}_{K} = \mathfrak{p}\)，本身就是素理想，不分解；
\item
  \textbf{分裂素}：\(p \equiv \pm 1\quad(mod\ 5)\)，
  \(p\mathcal{O}_{K} = \mathfrak{p} \cdot \overline{\mathfrak{p}},\)
  \(\mathfrak{p},\overline{\mathfrak{p}}\)是两个\textbf{不同的本原素理想}，且二者范数相等：
...
\textbf{D.2 分裂位置的局部域与群直积分解}

取分裂素
\( p\equiv\pm1 \pmod{5} \)，做局部完备化：
...
\textbf{D.6 和 BSD 论文（模曲线}
\(X_{0}(N)\)\textbf{）的平行对应（文稿互通）}

这是保证系列论文术语统一的对照，方便跨文档阅读：

\textbar{} 三维实二次域 RH
文稿（\(K = \mathbb{Q}\left( \sqrt{5} \right)\)） \textbar{} BSD
定性文稿（模曲线 \(X_{0}(N)\)） \textbar{}

\textbar-\/-\/-\textbar-\/-\/-\textbar{}

\textbar{} 有理素在 K 分裂：\(p \equiv \pm 1\quad(mod\ 5)\) \textbar{}
椭圆曲线分裂乘法坏约化素 \(p \mid \Delta_{E}\)\textbar{}

\textbar{} 局部域 \(K_{p} \cong \mathbb{Q}_{p} \oplus \mathbb{Q}_{p}\)
\textbar{} 局部域
...
\textbar{} 两条本原闭测地线
\(\gamma_{\mathfrak{p}}\),\(\gamma_{\overline{\mathfrak{p}}}\)
\textbar{} 两条等长本原闭测地线 \(\gamma_{p}^{(1)},\gamma_{p}^{(2)}\)
\textbar{}

\textbar JL 局部酉投影归一因子 \(w_{p} = \frac{1}{2}\) \textbar{} JL
局部酉投影归一因子 \(w_{p} = \frac{1}{2}\)\textbar{}

\textbar{} 惯性 / 分歧素，\(w_{p} = 1\) \textbar{}
好约化、加性坏约化素，\(w_{p} = 1\) \textbar{}
...
\def\labelenumi{\arabic{enumi}.}
\item
  分裂有理素在实二次域中分解为一对共轭素理想，保序双射给出两条等长本原闭测地线，造成几何侧求和出现局部因子
  2；
\item
  分裂素局部域直积分解，\({GL}_{2}\left( K_{p} \right)\)
  表示写成外张量积，对应两份正交局部表示，分别对应两条测地线；
\item
  Jacquet‑Langlands 局部提升做酉正交投影，把两份表示空间压入单份
  \({GL}_{2}\left( \mathbb{Q}_{p} \right)\) 表示，投影归一化给出
  \(w_{p} = \frac{1}{2}\)；
\item
  惯性、分歧素无直积分解，投影不改变内积，\(w_{p} = 1\)；
\item
  逐项验算：加权之后分裂素局部贡献等价于单份轨道权重，全局求和等价于遍历有理素的单层求和，与
  L 函数欧拉乘积的素遍历结构匹配；
\item
  整套局部机制可以完整平移到模曲线 \(X_{0}(N)\)
